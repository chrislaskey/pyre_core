defmodule Pyre.Flows.Chat do
  @moduledoc """
  Single-agent chat flow.

  A minimal flow with one generalist stage that can handle any task
  interactively. This is the default workflow for the web UI.

      generalist -> complete

  ## Usage

      Pyre.Flows.Chat.run("Help me debug this issue")
  ...
  The Chat flow is interactive by default — the generalist stage pauses
  for user input after its initial response.
  """
  @spec default_interactive_stages() :: [atom()]
  def default_interactive_stages, do: [:generalist]
