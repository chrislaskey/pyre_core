defmodule Pyre.Flows.Task do
  @moduledoc """
  Single-stage task flow.

  Runs a generalist agent that handles the task from start to finish:

      tasking -> complete

  Not interactive by default — the agent runs to completion without
  pausing for user input.

  ## Usage

      Pyre.Flows.Task.run("Add pagination to the products page")
  ...
  defp drive(%{phase: :tasking} = state, context) do
    with {:ok, result} <-
  ...
  @stage_fallback_field %{
    generalist: :task_output
  }

  @stage_artifact_info %{
    generalist: {:task_output, "01_task_output"}
  }
